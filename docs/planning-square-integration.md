# 📅 Planning — Square Appointments + intégration site (tâche connexe)
...
```
Bloc A — Configuration Square Appointments                (~90 min, Louis)
   ↓
...
## Bloc A — Configuration Square Appointments (👤 Louis)
...
| # | Tâche | Temps | Notes |
|---|---|---|---|
| A1 | Créer compte Square Appointments sur squareup.com/appointments | 10 min | Email `blackandbeauty.studio@gmail.com`, mdp fort stocké dans Bitwarden. Activer 2FA. |
...
| A8 | Copier-coller les 3 politiques (annulation, retard, no-show) | 5 min | Textes prêts dans `docs/politiques-rdv.md` |
| A9 | Activer rappels **email** auto 24 h avant | 5 min | Notifications → Reminders. SMS auto = Square Appointments Plus (~29 $/mois), pas dans le free tier. |
| A10 | Connecter Google Calendar d'Aalie | 10 min | Nécessite son consentement OAuth ; à faire avec elle si possible |
| A11 | Récupérer le **code d'embed** du widget | 5 min | Dans Square Appointments : Online Booking → Booking channels → Website → « Add Book Now button » ou « Embed booking widget ». Square propose 2 formats : lien direct `squareup.com/appointments/book/...` OU snippet iframe. Prendre l'URL du lien direct pour flexibilité. |
| A12 | Envoyer l'URL du widget à Claude | 1 min | Coller ici, on l'utilise pour l'intégration |

**Livrable de sortie** : URL du widget Square Appointments.
...
|---|---|---|---|
| B1 | Retravailler `public/reservation.html` — remplacer le bouton IG par le widget Square Appointments + fallback IG | 30 min | `reservation.html` |
...
| B5 | Mettre à jour la `Content-Security-Policy` dans `vercel.json` : `frame-src https://booking.squareup.com/appointments https://*.squareup.com/appointments` | 5 min | `vercel.json` |
| B6 | Mettre à jour `politique-confidentialite.html` — mentionner Square Appointments comme sous-traitant (Loi 25) | 15 min | `politique-confidentialite.html` |
...
| C5 | Créer un signet du tableau de bord Square Appointments dans le navigateur d'Aalie | 2 min | 👤 |
...
| D4 | Louis met à jour les services dans Square Appointments avec les vrais chiffres | 👤 |
| D5 | Aalie confirme ses horaires réels d'ouverture | 💅 |
| D6 | Louis ajuste les horaires dans Square Appointments | 👤 |
...
| Jour | Actions |
|---|---|
| **J** (aujourd'hui, 13 sept.) | Louis boucle Bloc A (compte Square Appointments, 90 min) + envoie l'URL widget à Claude |
...
| Risque | Mitigation |
|---|---|
| Aalie prend plusieurs jours à valider les politiques | On met en prod avec les politiques proposées ; ajustement instantané dès qu'elle répond (une modif texte dans Square Appointments = 30 secondes) |
| Widget Square Appointments ne charge pas dans certaines conditions (JS off, adblocker, réseau bloquant) | Fallback IG DM visible en permanence sous le widget |
| Google Calendar d'Aalie non connecté au moment du 1er RDV | Square Appointments fonctionne quand même ; sync activée dès qu'elle donne son OAuth |
| Frais SMS Square Appointments dépassent le tier gratuit (rare) | Alerte configurée dans Square Appointments ; possible upgrade Square Appointments Premium à 12 $/mois si nécessaire |
